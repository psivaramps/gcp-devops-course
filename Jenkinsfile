pipeline {
  ...
  stages {
	...
	//sh 'cd Jenkins'		 
    	...
    	sh 'gcloud compute instances create springapp-dev --zone=asia-south1-a --machine-type=e2-small --network-interface=network=vpc-automode --tags=http-server --metadata-from-file=startup-script=./scripts/startup-script.sh'
  ...
  stage ('DEV App health check') {
	...
	steps {
     	...
     	sh 'gcloud config set project envmnt-uat'
      	sh 'gcloud compute instances create springapp-uat --zone=us-central1-a --machine-type=e2-small --tags=http-server --metadata-from-file=startup-script=./scripts/startup-script.sh'
	...
	steps {
     	...
     	sh 'gcloud compute instances create springapp-prod --zone=us-central1-a  --machine-type=e2-small --tags=http-server --metadata-from-file=startup-script=./scripts/startup-script.sh'

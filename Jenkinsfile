pipeline {
  ...
  stage ('DEV App health check') {
	...
	steps {
     	...
     	sh 'gcloud compute instances create springapp-prod --zone=central1-a  --machine-type=e2-small --tags=http-server --metadata-from-file=startup-script=./scripts/startup-script.sh'
